...
def _recall(y_true: Sequence[ItemIdType], y_pred: Sequence[ItemIdType], k: int) -> float:
    if len(y_true) == 0 or len(y_pred) == 0:
        return 0
    ...
    return numpy.isin(y_pred[:k], y_true, assume_unique=True).sum() / min(k, len(y_true))
...
def recall_score(y_true: numpy.ndarray, y_pred: numpy.ndarray, k: int) -> float:
    y_true = check_array(y_true, dtype=numpy.object_, ensure_2d=False)
    y_pred = check_array(y_pred, dtype=numpy.object_, force_all_finite='allow-nan', ensure_2d=False)
    check_consistent_length(y_true, y_pred)
    ...
    scores = []
    for i in range(y_true.shape[0]):
        scores.append(_recall(y_true[i], y_pred[i], k))
